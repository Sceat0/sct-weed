    ...
    if not topluyormu then
        ...
        exports['mythic_progbar']:Progress({
            ...
            if not cancelled then
                TriggerServerEvent('sct-weed:giveweed')
                topluyormu = false
...
function EsrarIsle()
    if not isliyormu then
        ESX.TriggerServerCallback('sct-weed:malzemekontrol', function(data)
            local player = PlayerPedId()
            isliyormu = true
            local finished = exports["reload-skillbar"]:taskBar(5000,math.random(5,15))
            if finished ~= 100 then
                TriggerServerEvent('sct-weed:removeitem', 'esrar_ham', 1)
                isliyormu = false
                ...
                if finished2 ~= 100 then
                    ...
                    if finished3 ~= 100 then
                        ...
                        TriggerServerEvent('sct-weed:processweed')
                        isliyormu = false
...
function EsrarIsle2()
    if not kurutuyormu then
        ESX.TriggerServerCallback('sct-weed:malzemekontrol', function(data)
            ...
            if finished ~= 100 then
                TriggerServerEvent('sct-weed:removeitem', 'esrar_islenmis', 1)
                kurutuyormu = false
                ClearPedTasksImmediately(player)
                ...
                if finished2 ~= 100 then
                    ...
                    if finished3 ~= 100 then
                        ...
                        TriggerServerEvent('sct-weed:processweed2')
                        kurutuyormu = false
                        ClearPedTasks(player)
    ...
    if not paketliyormu then
        ESX.TriggerServerCallback('sct-weed:malzemekontrol', function(data)
            ESX.TriggerServerCallback('sct-weed:malzemekontrol', function(data)
                ...
                if finished ~= 100 then
                    TriggerServerEvent('sct-weed:removeitem', 'esrar_islenmis2', 1)
                    paketliyormu = false
                    ClearPedTasksImmediately(player)
                    ...
                    if finished2 ~= 100 then
                        ...
                        if finished3 ~= 100 then
                            ...
                            TriggerServerEvent('sct-weed:packweed')
                            paketliyormu = false
                            ClearPedTasks(player)
